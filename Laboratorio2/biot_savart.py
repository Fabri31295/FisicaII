    ...
    def graficar_lineas_campo_combinado(self):
        ...
        for i in range(X_vec.shape[0]):
            for j in range(X_vec.shape[1]):
                ...
                # Evitar puntos muy cerca de las estructuras
                dist_alambre = np.sqrt(x_pos**2 + y_pos**2)
                dist_espira = abs(np.sqrt(x_pos**2) - 3.0)
                
                if dist_alambre < 0.3 or (dist_espira < 0.4 and abs(z_pos) < 0.2):
                    continue
                
                self.punto = np.array([x_pos, y_pos, z_pos])
                B_alambre, B_espira, B_total = self.campo_combinado(N=300)
        ...
        # Normalizar vectores
        B_mag = np.sqrt(Bx_total**2 + By_total**2 + Bz_total**2)
        B_mag[B_mag == 0] = 1e-10
        
        scale_factor = 0.3
        ax2.quiver(X_vec, Y_vec, Z_vec, 
                  Bx_total/B_mag*scale_factor, By_total/B_mag*scale_factor, Bz_total/B_mag*scale_factor,
                  color='darkblue', alpha=0.8, arrow_length_ratio=0.2)
